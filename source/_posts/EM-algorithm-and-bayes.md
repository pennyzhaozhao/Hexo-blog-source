...
$$
\hat{\mu}_k = \frac{1}{n_k} \sum_{i=1}^{n} \mathbf{1}_{\{\hat{y}_i = k\}} x_i
= \frac{1}{n_k} \sum_{i \in G_k} x_i
$$

其中：

- $\mathbf{1}_{\{\hat{y}_i = k\}}$ 是指示函数，表示样本 $x_i$ 是否被分配到类别 $k$。
- $G_k = \{ i \mid \hat{y}_i = k \}$ 表示所有被分配到类别 $k$ 的样本集合。
- $n_k = |G_k|$ 表示类别 $k$ 中的样本数。
